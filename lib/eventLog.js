...
async function record(clientId, event) {
  await db.query(
    `INSERT INTO events (client_id, comment_id, text, verdict, deleted, error, platform, author, author_id, auto_blocked, post_url, author_url)
...
/** Same shape as stats(), but summed across every client -- for the admin overview. */
...
/** Most recent events across every client, with the client's name attached -- for the admin overview's activity chart/table. */
async function recentAcrossClients(limit = 300) {
  const { rows } = await db.query(
    `SELECT events.*, clients.name AS client_name
     FROM events JOIN clients ON clients.id = events.client_id
     ORDER BY events.created_at DESC LIMIT $1`,
    [limit]
  );
  return rows.map((row) => ({ ...toEvent(row), clientName: row.client_name }));
}

export { record, list, stats, globalStats, recentAcrossClients, getByCommentId, markDeleted };

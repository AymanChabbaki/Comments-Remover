...
async function fetchPages(userAccessToken) {
  ...
  if (!data.data || data.data.length === 0) {
    console.error('Facebook /me/accounts returned no Pages, raw response:', JSON.stringify(data));
  }
  return data.data || [];
}

/**
 * Diagnostic helper -- lists which permissions the user token actually
 * ended up with (granted vs declined), so a silently-empty /me/accounts
 * response can be told apart from "the token never got pages_show_list
 * at all" vs "it did, but /me/accounts still returned nothing".
 */
async function fetchGrantedPermissions(userAccessToken) {
  const version = process.env.GRAPH_API_VERSION || 'v19.0';
...
async function subscribePageToWebhooks(pageId, pageAccessToken) {
  ...
  try {
    await axios.post(`https://graph.facebook.com/${version}/${pageId}/subscribed_apps`, null, {
      params: { subscribed_fields: 'feed,messages', access_token: pageAccessToken },
    });
  } catch {
    console.warn('Messaging subscription unavailable. Grant pages_messaging and reconnect; comment moderation remains subscribed.');
  }
}

export { exchangeCodeForUserToken, fetchPages, fetchGrantedPermissions, subscribePageToWebhooks };

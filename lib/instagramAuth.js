 ...
 * uses the `instagram_business_basic` scope granted alongside comment
 * management. Shown in Settings so a client can confirm the right account
 * connected, and is also what the App Review screencast demonstrates.
 */
async function fetchProfile(userId, accessToken) {
  const version = process.env.GRAPH_API_VERSION || 'v19.0';
  const { data } = await axios.get(`https://graph.instagram.com/${version}/${userId}`, {
    params: { fields: 'username,profile_picture_url', access_token: accessToken },
  });
  return { username: data.username, profilePicUrl: data.profile_picture_url };
}
 ...
 */
async function subscribeToWebhooks(userId, accessToken) {
  const version = process.env.GRAPH_API_VERSION || 'v19.0';
  await axios.post(`https://graph.instagram.com/${version}/${userId}/subscribed_apps`, null, {

 ...
 */
async function block(clientId, platform, authorId, authorName, reason, authorUrl = null) {
  if (!authorId) return;
  await db.query(
    `INSERT INTO blocklist (client_id, platform, author_id, author_name, reason, author_url)
     VALUES ($1, $2, $3, $4, $5, $6)
     ON CONFLICT (client_id, platform, author_id)
     DO UPDATE SET author_name = EXCLUDED.author_name, reason = EXCLUDED.reason, author_url = EXCLUDED.author_url, blocked_at = now()`,
    [clientId, platform, authorId, authorName || null, reason || null, authorUrl]
  );
}
...
export { isBlocked, block, unblock, list };

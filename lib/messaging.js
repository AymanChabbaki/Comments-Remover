...
export async function queueRules(client, platform, trigger, eventId, recipient, text, at = new Date(), meta = {}) {
  ...
  // Facebook's /private_replies edge only exists on a TOP-LEVEL comment --
  // a reply-to-a-reply has no such edge at all, and fails with the same
  // generic "does not exist / missing permissions" error Graph API gives
  // for a dozen unrelated problems. Catching it here means the failure
  // reason a client sees in Message History is actually useful.
  const precheck = platform === 'facebook' && kind === 'private' && meta.isNestedReply
  ...
  await enqueue(client, platform, kind, trigger === 'incoming' ? recipient : eventId, rule.message,
    `event:${key}`, new Date(new Date(at).getTime() + (kind === 'dm' ? 24 : 7 * 24) * 3600000), precheck);
}

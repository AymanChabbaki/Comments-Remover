...
// Note on Facebook commenter profile links: there is no way to resolve
// `from.id` on a Page comment to a real profile URL. Confirmed against
// live data, not just theory -- GET /{that-id}?fields=link comes back
// with error_subcode 33 ("this ID/token combination cannot be queried"),
// Meta's own documented code for a scoped ID being rejected outright, not
// a missing field. The ID is deliberately opaque to third-party apps.
// There's nothing left to try here via the Graph API; a link component
// just shows the name as plain text for Facebook, real profile links for
// Instagram only (its comments carry a real username, not a scoped ID).

export { getCommentText, deleteComment, getPostPermalink };
